...
```{r mean by continent 2}
#lifeExp_by_country <- aggregate(gapminder$lifeExp~gapminder$continent, FUN=mean)
#for (i in 1:nrow(lifeExp_by_country)){
#   print(lifeExp_by_country[i, ])
#}
```
...
for (c in conts){
    for (y in years){
        print(paste(c, y, mean(gapminder$lifeExp[gapminder$continent == c & gapminder$year == y])))
    }
}
```
...
``` {r gap in lifeExp between countries on different continents}
conts <- unique(gapminder$continent)
years <- unique(gapminder$year)
...
for (y in years){
    print(paste('year', y))
    for (c in conts){
        sd_cy = sd(gapminder$lifeExp[gapminder$continent == c
                                     & gapminder$year == y])
        print(paste('continent', c, sd_cy))
    }
}
...
y <- 1987
while (y <= 2002){
    ans <- sd(gapminder$lifeExp[gapminder$year == y])
    print(paste0(y, ': ', ans))
    y <- y + 5
}
