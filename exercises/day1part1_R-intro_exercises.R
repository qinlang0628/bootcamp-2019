...
subset(gapminder07, subset = country=='Mexico')$pop
...
hist(gapminder07$lifeExp, xlab = 'lifeExp', ylab = 'number of countries', main = 'life expectancy histogram')

# Bonus: Change the `breaks = ` argument from its default setting and see what happens.
hist(gapminder07$lifeExp, 
     xlab = 'lifeExp', 
     ylab = 'number of countries', 
     main = 'life expectancy histogram', 
     breaks = 3)
...
plot(lifeExp~gdpPercap, data = gapminder07, main='life expentancy over gdp per cap', xlab='GDP per cap', ylab='life expenctacy')
